...
def getDataFromEdwardJonesOld(file, userID, passCode):
...
def getDataFromEdwardJones(file, userID, passCode):
    ...
    driver.get(
        'https://accountaccess.edwardjones.com/ca-logon/logon.action')
    elemN = driver.find_element_by_id("userNa")
    elemN.clear()
    elemN.send_keys(userID)

    elemP = driver.find_element_by_id("passwd")
    elemP.clear()
    elemP.send_keys(passCode)

    elemB = driver.find_element_by_id("login-uid")
    elemB.click()
