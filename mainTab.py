...
class Root(tk.Tk):
    ...
    def addFilesTab(self, tab):
        ...
        frame = Frame(tab)
        frame.grid(row = 2, column = 0, columnspan = 4, padx=5, pady=5, sticky=tk.NSEW)

        scrollbar = Scrollbar(frame, orient=tk.VERTICAL)
        self.Lb1 = Listbox(frame, selectmode=tk.EXTENDED, width = 32, yscrollcommand=scrollbar.set)
        ...
        status = Label(tab, text="Status: Total Files:99 Selected Files:18")
        status.grid(row=4,column=0, sticky=tk.S+tk.W, columnspan = 4)
