...
class Root(tk.Tk):
    ...
    def __init__(self):
        ...
        lbl = Label(f, text="Keith")
        lbl.grid(row = 0, column=0, sticky='ns')
        f.rowconfigure(0, weight=0)
        #fFiles = dsFiles.dsfiles(self)
        #fFiles.getFrame().grid(row=0, column=0, sticky='ns')
        #fFiles.getFrame().rowconfigure(0, weight=1)
        
        
        #fFiles1 = dsFiles.dsfiles(self)
        #fFiles1.getFrame().grid(row=0, column=1)
        #fDsevents = dseventsFrame.dsevents(self)
        #fDsevents.getFrame().grid(row=0,  column=1)
...
createMenu(window)
f1 = basicLabels(window)
f = dsFiles.dsFiles(window)
f.getFrame().grid(row=0, column=0, padx=5, pady=5,stick='NS')
